...
export function generateReviewsForProduct(productId: number): ProductReview[] {
  ...
  return names.map((reviewerName, index) => {
    ...
    return {
      ...
      status: "approved",
      // Only line changed on restore. ProductReview now requires a source, and
      // this data has none of the real ones -- "storefront" would assert a
      // customer typed it. Tags every generated row as a fixture.
