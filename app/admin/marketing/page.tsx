  ...
  return (
    <AdminShell title="Marketing Autopilot">
      ...
      <section className="mt-8 rounded-lg bg-white shadow-sm border border-zinc-200 p-6">
        ...
        <p className="text-sm text-zinc-600">{bundle.decisions.summary}</p>
      </section>
        ...
        <section className="rounded-lg bg-white shadow-sm border border-zinc-200 p-6">
          ...
          <ul className="space-y-2 text-sm">
            {bundle.viral.products.length === 0 ? (
              <li className="text-zinc-600">No viral signals yet.</li>
            ) : (
              bundle.viral.products.map((product) => (
                <li key={product.productId} className="flex justify-between gap-4">
                  <span>{product.name}</span>
                  <span className="text-accent">{product.viralScore}/100</span>
                </li>
              ))
            )}
          </ul>
          ...
          {bundle.autopilot.ads.slice(0, 9).map((ad, index) => (
            <article key={`${ad.platform}-${ad.productId}-${index}`} className="rounded-lg border border-zinc-100 p-4 text-sm">
              <div className="mb-2 flex items-center justify-between gap-3">
                <span className="font-semibold uppercase text-accent">{ad.platform}</span>
                <span className="text-gray-500">Product #{ad.productId}</span>
              </div>
              <p className="font-medium">{ad.hook}</p>
              <p className="mt-2 text-zinc-600">{ad.adCopy}</p>
        ...
        <section className="rounded-lg bg-white shadow-sm border border-zinc-200 p-6">
          ...
          <ul className="space-y-3 text-sm">
            {bundle.content.packs.slice(0, 4).map((pack) => (
              <li key={pack.productId} className="rounded-lg border border-zinc-100 p-3">
                <p className="font-medium">{pack.productName}</p>
                <p className="mt-2 line-clamp-2 text-zinc-600">{pack.tiktokCaption}</p>
              </li>
            ))}
          </ul>
        </section>
          ...
          <ul className="space-y-2 text-sm">
            {bundle.behavior.segments.slice(0, 4).map((segment) => (
              <li key={segment.segment} className="flex justify-between gap-4">
                ...
                <span className="text-zinc-600">{segment.purchaseLikelihood}% likely</span>
              </li>
            ))}
          </ul>
        </section>
