...
export default async function AdminOrderDetailPage({ params }: PageProps) {
  ...
  return (
    <AdminShell title={`Order ${order.order_number}`}>
      <div className="mb-6">
        <Link href="/admin/orders" className="text-sm text-red-600 hover:text-red-700">
          ← Back to all orders
        </Link>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <div className={adminUi.card}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="text-xl font-semibold text-zinc-900">{order.order_number}</p>
                <p className="mt-1 text-xs text-zinc-500">Placed {formatTime(order.created_at)}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-zinc-600">Total</p>
                <p className="text-2xl font-bold text-zinc-900">${Number(order.total).toFixed(2)}</p>
              </div>
            </div>
            ...
            {order.customer ? (
              <div className="mt-4 border-t border-zinc-100 pt-4 text-sm text-zinc-700">
                ...
                {order.customer.shipping_address ? <p>{order.customer.shipping_address}</p> : null}
              </div>
            ) : null}
          ...
          <div>
            <h2 className="mb-3 text-sm font-semibold text-zinc-900">Status controls</h2>
        ...
        <div className={adminUi.card}>
          ...
          {events.length === 0 ? (
            <p className="mt-3 text-xs text-zinc-500">No events recorded yet.</p>
          ) : (
