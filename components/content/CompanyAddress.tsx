...
const variantClass: Record<CompanyAddressVariant, string> = {
  summary: "space-y-1 text-sm text-gray-400",
  "us-hq": "space-y-0.5 text-sm text-gray-300 leading-relaxed",
  "japan-hub": "space-y-0.5 text-sm text-gray-300 leading-relaxed",
};
...
export default function CompanyAddress({
  ...
  if (variant === "summary") {
    return (
      <address className={`${classes} not-italic`}>
        <p className="font-semibold text-white">{COMPANY_LOCATION_SUMMARY.brand}</p>
  ...
  if (variant === "us-hq") {
    return (
      <address className={`${classes} not-italic`}>
        <p className="font-medium text-white">{US_HEADQUARTERS.companyName}</p>
  ...
  return (
    <address className={`${classes} not-italic`}>
      <p className="font-medium text-white">{JAPAN_LOGISTICS_HUB.companyName}</p>
